...
function PetrickMethod()
{
  ...
  this.test = function() {
    ...
    var monomB;
    orArray = [];
    monomA = {}; // using objects ensures that (x and x) = x
    monomA[1] = 1;
    orArray.push(monomA);
    monomB = {};
    monomB[2] = 2;
            ...
            for (var thisVal in primTerm.implicant.imp) {
              ...
              for (var v = 0; v < this.noOfVars; v++) {
                if ((needed & one) === one) {
                  if ((thisVal & one) === one) {
                    if(v == 0){
                      minTerm = "<i>B</i>" + minTerm;
                    }else if(v == 1){
                      minTerm = "<i>D</i>" + minTerm;
                    }else if(v == 2){
                      minTerm = "<i>A</i>" + minTerm;
                    }else if(v == 3){
                      minTerm = "<i>C</i>" + minTerm;
                    }
                  }
                  else {
                    if(v == 0){
                      minTerm = "<i>B&#772;</i>" + minTerm;
                    }else if(v == 1){
                      minTerm = "<i>D&#772;</i>" + minTerm;
                    }else if(v == 2){
                      minTerm = "<i>A&#772;</i>" + minTerm;
                    }else if(v == 3){
                      minTerm = "<i>C&#772;</i>" + minTerm;
                    }
                  }
                }
                one = one << 1;
